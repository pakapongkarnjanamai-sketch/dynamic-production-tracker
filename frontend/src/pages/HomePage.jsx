...
export default function HomePage() {
  ...
  const { data, loading, error, reload } = useAsyncData(
    useCallback(async () => {
      const [lines, stats, traySummary] = await Promise.all([
        getLines(),
        getTrayStats(),
        getLogsSummary(),
      ]);

      return { lines, stats, traySummary };
    }, []),
    {
      initialData: {
        lines: [],
      ...
      getErrorMessage: (err) => err?.message || "โหลดข้อมูลหน้าหลักไม่สำเร็จ",
    },
  );

  const { lines, traySummary, stats } = data;
