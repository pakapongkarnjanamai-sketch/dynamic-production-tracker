...
import { createUser, deleteUser, getOperators, getUsers, updateUser } from '../../api/client';
import {
	AdminSection,
	Badge,
	Button,
...
export default function UsersSection({ currentRole }) {
	...
	const loadData = async () => {
		try {
			setError('');
			setLoading(true);
			const [userData, operatorData] = await Promise.all([getUsers(), getOperators()]);
			setUsers(userData);
	...
	return (
		...
		>
			{loading ? <LoadingState message="กำลังโหลดข้อมูลบัญชีผู้ใช้..." /> : null}
			{!loading && error ? <ErrorState message={error} onRetry={loadData} /> : null}
			{!loading && !error && filteredUsers.length === 0 ? (
				<EmptyState
					title={users.length === 0 ? 'ยังไม่มีบัญชีผู้ใช้' : 'ไม่พบบัญชีที่ค้นหา'}
					description={
						users.length === 0
							? 'สร้างบัญชีเพื่อกำหนดสิทธิ์และผูกกับข้อมูลพนักงานในระบบ'
							: 'ลองเปลี่ยนคำค้น หรือสร้างบัญชีใหม่'
					}
					action={users.length === 0 ? <Button onClick={openCreateModal}>สร้างบัญชีแรก</Button> : null}
				/>
			) : null}
					...
					<Stack className="md:hidden">
						{filteredUsers.map((user) => (
							<MobileCard key={user.id}>
								...
								<div className="mt-4">
									<FormActions>
									<Button size="compact" variant="secondary" className="flex-1" onClick={() => openEditModal(user)}>
										แก้ไข
									</Button>
									</FormActions>
								</div>
			...
			>
				<form className="space-y-4" onSubmit={handleSubmit}>
					<Input label="รหัสประจำตัว (Login ID) *" value={form.employeeId} onChange={handleChange('employeeId')} required />
					<Input label="ชื่อผู้ใช้งาน *" value={form.name} onChange={handleChange('name')} required />
					<Input
						...
						required={!editUser}
					/>
					<Input as="select" label="สิทธิ์ (Role) *" value={form.role} onChange={handleChange('role')}>
					...
					<Input as="select" label="ผูกกับ Profile พนักงาน (ถ้ามี)" value={form.operatorId} onChange={handleChange('operatorId')}>
					...
					<FormActions>
						<Button type="submit" className="flex-1" disabled={submitting}>
							{submitting ? 'กำลังบันทึก...' : editUser ? 'บันทึกข้อมูล' : 'สร้างบัญชี'}
						</Button>
						{editUser ? (
							<Button type="button" variant="danger" className="flex-1" onClick={() => handleDelete(editUser.id)}>
								ลบข้อมูล
							</Button>
						) : null}
