...
export default function LinesSection({ lines, onRefresh, loading = false, error = '' }) {
	...
	return (
		...
		>
			...
			{!loading && !error && filteredLines.length === 0 ? (
				<EmptyState
					title={lines.length === 0 ? 'ยังไม่มีสายการผลิต' : 'ไม่พบสายการผลิตที่ค้นหา'}
					description={
						lines.length === 0
							? 'สร้างสายการผลิตก่อน เพื่อกำหนด process ให้กับงานแต่ละรายการ'
							: 'ลองเปลี่ยนคำค้น หรือเพิ่มสายการผลิตใหม่'
					}
					action={lines.length === 0 ? <Button onClick={openCreateModal}>เพิ่มสายการผลิตแรก</Button> : null}
				/>
			) : null}
					...
					<Stack className="md:hidden">
						{filteredLines.map((line) => (
							<MobileCard key={line.id}>
								...
								<div className="mt-4">
									<FormActions>
									<Button size="compact" variant="secondary" className="flex-1" onClick={() => openProcessModal(line)}>
										จัดการขั้นตอน
									</Button>
									<Button size="compact" variant="text" className="flex-1" onClick={() => openEditModal(line)}>
										แก้ไข
									</Button>
									</FormActions>
								</div>
			...
			>
				<form className="space-y-4" onSubmit={handleSubmit}>
					<Input label="ชื่อสายการผลิต *" value={form.name} onChange={handleChange('name')} required />
					<Input as="textarea" rows={3} label="รายละเอียด" value={form.description} onChange={handleChange('description')} />
					<FormActions>
						<Button type="submit" className="flex-1" disabled={submitting}>
							{submitting ? 'กำลังบันทึก...' : editLine ? 'บันทึกข้อมูล' : 'สร้างสายการผลิต'}
						</Button>
						{editLine ? (
							<Button type="button" variant="danger" className="flex-1" onClick={() => handleDelete(editLine.id)}>
								ลบข้อมูล
							</Button>
						) : null}
						<Button type="button" variant="secondary" className="flex-1" onClick={closeLineModal}>
							ยกเลิก
						</Button>
					</FormActions>
			...
			<Modal
				title={selectedLine ? `ขั้นตอนของ ${selectedLine.name}` : 'จัดการขั้นตอน'}
...
function ProcessManager({ lineId }) {
	...
	return (
		<div className="space-y-5">
			<form className="space-y-4 rounded-[24px] border border-neutral-200 bg-neutral-50 p-4" onSubmit={handleSubmit}>
				...
				<div className="grid grid-cols-1 gap-4 sm:grid-cols-[120px_minmax(0,1fr)]">
					<Input
						...
						required
					/>
					<Input label="ชื่อขั้นตอน *" value={form.name} onChange={handleChange('name')} required />
				</div>
				<Input as="textarea" rows={3} label="รายละเอียด" value={form.description} onChange={handleChange('description')} />
				<div className="mt-4">
					<FormActions>
					<Button type="submit" className="flex-1" disabled={submitting}>
						{submitting ? 'กำลังบันทึก...' : editId ? 'บันทึกการแก้ไข' : '+ เพิ่มขั้นตอน'}
					</Button>
					{editId ? (
						<Button type="button" variant="danger" className="flex-1" onClick={() => handleDelete(editId)}>
							ลบข้อมูล
						</Button>
					) : null}
					{editId ? (
			...
			{!loading && error ? <ErrorState message={error} onRetry={loadProcesses} /> : null}
