...
import { createTray, deleteTray, getLogs, getTrays, updateTray } from '../../api/client';
import {
	AdminSection,
	Badge,
	Button,
...
export default function TraysSection({ lines }) {
	...
	const openEditModal = (tray) => {
		...
		setForm({
			...
			due_date: tray.due_date ? new Date(tray.due_date).toISOString().slice(0, 16) : '',
		});
		setMessage('');
		setIsModalOpen(true);
	};
	...
	return (
		...
		>
			...
			{!loading && error ? <ErrorState message={error} onRetry={loadTrays} /> : null}
			{!loading && !error && filteredTrays.length === 0 ? (
				<EmptyState
					...
					action={trays.length === 0 ? <Button onClick={openCreateModal}>สร้างงานแรก</Button> : null}
				/>
			) : null}
					...
					<Stack className="md:hidden">
						{filteredTrays.map((tray) => (
							<MobileCard key={tray.id}>
								...
								<div className="mt-4">
									<FormActions>
									<Button size="compact" variant="secondary" className="flex-1" onClick={() => openLogs(tray)}>
										ดูประวัติ
									</Button>
									<Button size="compact" variant="text" className="flex-1" onClick={() => openEditModal(tray)}>
										แก้ไข
									</Button>
									</FormActions>
								</div>
			...
			>
				<form className="space-y-4" onSubmit={handleSubmit}>
					<Input
						...
						className="font-mono"
					/>
					<Input as="select" label="สายการผลิต" value={form.line_id} onChange={handleChange('line_id')}>
					...
					<Input label="สินค้า (Product)" value={form.product} onChange={handleChange('product')} />
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
						<Input label="Batch No." value={form.batch_no} onChange={handleChange('batch_no')} />
						<Input label="จำนวน (Qty)" type="number" min="1" value={form.qty} onChange={handleChange('qty')} />
					</div>
					<Input as="select" label="สถานะ" value={form.status} onChange={handleChange('status')}>
					...
					<FormActions>
						<Button type="submit" className="flex-1" disabled={submitting}>
							{submitting ? 'กำลังบันทึก...' : editTray ? 'บันทึกข้อมูล' : 'สร้างงาน'}
						</Button>
						{editTray ? (
							<Button type="button" variant="danger" className="flex-1" onClick={() => handleDelete(editTray.id)}>
								ลบข้อมูล
							</Button>
						) : null}
			...
			<Modal
				title={selectedTray ? `ประวัติ: ${selectedTray.qr_code}` : 'ประวัติการทำงาน'}
...
function TrayLogs({ trayId }) {
	...
	if (logs.length === 0) {
		return <EmptyState title="ยังไม่มีประวัติการทำงาน" description="ระบบยังไม่มี log สำหรับงานนี้" />;
	}
