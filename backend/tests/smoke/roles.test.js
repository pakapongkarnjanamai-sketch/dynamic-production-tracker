...
test('superadmin can manage all roles', () => {
  ...
  assert.equal(canManageRole(ROLES.SUPERADMIN, ROLES.OPERATOR), true);
  assert.equal(canManageRole(ROLES.SUPERADMIN, ROLES.VIEWER), true);
});

test('admin can manage operator/viewer only', () => {
  assert.equal(canManageRole(ROLES.ADMIN, ROLES.OPERATOR), true);
...
test('canAssignRole follows role manage policy', () => {
  assert.equal(canAssignRole(ROLES.ADMIN, ROLES.OPERATOR), true);
